...
export function validateJsonInput(input: string): ValidationResult {
  ...
  if (parsed === null) {
    errors.push({
      code: "NULL_VALUE",
      message: "Input evaluates to null. Provide a JSON array or object instead.",
    });
    return { valid: false, errors, warnings };
  }
  ...
  if (typeof parsed === "string" || typeof parsed === "number" || typeof parsed === "boolean") {
    warnings.push({
      ...
      message: "Input is a primitive value. It will be converted as a single-cell CSV row.",
    });
  }
