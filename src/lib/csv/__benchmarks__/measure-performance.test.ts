...
const SIZES = isQuick
    ? CSV_SIZES.filter((s) => s.rowCount <= 100_000)
    : CSV_SIZES;

// ── Helpers ──────────────────────────────────────

interface PerfResult { median: number; times: number[] }
...
interface SizeResult {
    label: string; rowCount: number; csvSizeKb: number;
    parseMs: number; validateMs: number; typeInfMs: number; pipelineMs: number;
}
...
describe("CSV Engine Performance", () => {
    ...
    beforeAll(() => {
        ...
        for (const size of SIZES) {
            ...
            console.log(`  ${size.label.padEnd(5)} ${size.rowCount.toLocaleString().padStart(10)} rows → ${(csv.length / 1024).toFixed(0).padStart(8)} KB  (gen: ${elapsed}s)`);
        }
        ...
        describe(`${size.label} (${size.rowCount.toLocaleString()} rows)`, () => {
            ...
            it("fullPipeline", () => {
                ...
                allResults.push({
                    ...
                    parseMs: 0, validateMs: 0, typeInfMs: 0,
...
afterAll(() => {
    ...
    console.log("Size    | Rows       | Size(KB) | Parse(ms) | Valid(ms) | TypeInf(ms) | Pipeline(ms)");
    console.log("────────┼────────────┼──────────┼───────────┼───────────┼─────────────┼──────────────");
