 ...
 * JSON Diff — engine.
 *
 * A line-based diff that is correct AND bounded:
...
/** LCS-based edit script over compare-key arrays. Indices are 0-based into the
 *  arrays passed in. Standard DP + backtrack — simple and provably correct. */
...
export function computeDiff(aStr: string, bStr: string, opts: DiffOptions): DiffResult {
    ...
    while (i < edits.length) {
        ...
        if (e.op === "equal") {
            ...
            for (const da of dels) unified.push({ type: "remove", na: da.a + 1, nb: null, text: aLines[da.a] });
            for (const ib of inss) unified.push({ type: "add", na: null, nb: ib.b + 1, text: bLines[ib.b] });
        }
    }
