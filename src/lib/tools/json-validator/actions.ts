...
export function handleInput(store: Store<JsonValidatorState>, text: string): void {
    ...
    if (!text.trim()) {
        store.set({ ...state, jsonInput: text, result: null, inputStatus: "empty", outputStatus: "empty", isValidating: false, error: null });
        return;
    }
...
export function validate(store: Store<JsonValidatorState>): void {
    ...
    const onMessage = (e: MessageEvent) => {
        ...
        if (data.ok && data.result) {
            ...
            store.update((s) => ({ ...s, isValidating: false, error: data.error ?? "Validation failed." }));
        }
    };
