...
function validateStructure(
    ...
    const here = (): Token =>
        tokens[p] ?? { type: "rbrace", start: s.length }; // EOF sentinel for messages
...
export function validateJson(input: string, opts: ValidatorOptions): ValidationResult {
    ...
    if (input.trim().length === 0) {
        return { valid: false, authoritative: false, size, error: { message: "Empty input", position: 0, line: 1, column: 1 } };
    }
    ...
    if (opts.includeNormalized) {
        ...
        normalized = JSON.stringify(JSON.parse(input), null, opts.indent === "tab" ? "\t" : Number(opts.indent));
    }
...
export function buildReport(result: ValidationResult, source: string): string {
    ...
    if (dup.length > 0) {
        out.push("");
        out.push("  ! " + dup.slice(0, 8).map((k) => JSON.stringify(k)).join(", ") + (dup.length > 8 ? ", …" : ""));
    }
