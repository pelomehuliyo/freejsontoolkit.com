...
export function handleInput(store: Store<JsonToCsvState>, text: string): void {
  ...
  if (text.length > MAX_INPUT_CHARS) {
    store.set({
      ...
      error:
        ...
        " chars). To keep the page responsive, limit is " +
  ...
  if (text.length > LARGE_FILE_THRESHOLD) {
    ...
    const preview =
      ...
      "\n\n... [Truncated: " +
      (text.length - PREVIEW_LENGTH).toLocaleString() +
      " more characters. Click Convert to process the entire file.]";
...
export async function convertJsonToCsv(
  ...
  if (trimmed.length > WARNING_THRESHOLD) {
    const msg =
      "The JSON data is large (" +
      (trimmed.length / 1024 / 1024).toFixed(1) +
      " MB). Converting it might take a few seconds and consume memory. Do you want to proceed?";
